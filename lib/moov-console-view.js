...
export default class MoovConsoleView {
  ...
  render() {
    ...
    for (let key in projs) {
      ...
      projectConfigDivs.push(<div key={this.index} index={this.index}>{ key }: <a href={projs[key]}>{ projs[key] } </a><button className="btn btn-remove" onClick={ removeMoovConfigItem.bind(this, key) }>x</button><br/></div>);
    }
